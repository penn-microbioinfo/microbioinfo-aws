...
class S3MultiPartUpload(object):
    ...
    def _generate_etag (self):
        if self.part_fnames is not None and self.part_digests is not None:
            self.etag =  f"{md5hash(b''.join(self.part_digests)).hexdigest()}-{len(self.part_digests)}"
        else:
    ...
    def _md5_digest_parts(part_fnames):
        return [md5hash(open(x, 'rb').read()).digest() for x in part_fnames]
...
def make_file_parts(largefile, part_size = 4950000000, parts_outdir = None): 
    ...
    try:
        ...
        logging.critical(f"Call to `split` failed with: {err}")
        sys.exit(1)
...
def md5hash(bytestream):
    hl = hashlib.new("md5")
    hl.update(bytestream)
    return hl
